...
- `stargate.ipfs.*`: IPFS uploads mirroring configuration (see values for defaults)
  ...
  - `stargate.ipfs.apiUrl`: optional Kubo HTTP API URL (default empty — embedded node only)
...
- `ipfs.*`: optional Kubo sidecar (retired; embedded node is the default)
  - `ipfs.enabled`: deploy Kubo service (default `false`)
